...
| 1.3     | Use RLS client instead of admin for reads             | Auth         | Low    | Low    | ✅ Done — 16 Sep 2026 | [performance/reivew-items](https://github.com/tommyarmstrong/football-team-organizer/pull/154) |
...
| 2.2     | Switch `useLayoutEffect` to `useEffect`               | Theming      | Low    | Low    | ✅ Done — 16 Sep 2026 | [performance/reivew-items](https://github.com/tommyarmstrong/football-team-organizer/pull/154) |
...
| **3.2** | Parallel `getUser()` + `has_app_access`               | Middleware   | Medium | Low    | ✅ Done — 16 Sep 2026 | [performance/reivew-items](https://github.com/tommyarmstrong/football-team-organizer/pull/157) |
| 3.3     | Narrow middleware matcher                             | Middleware   | Low    | Low    | ✅ Done — 16 Sep 2026 | [performance/reivew-items](https://github.com/tommyarmstrong/football-team-organizer/pull/154) |
...
| 4.2     | Split header into multiple Suspense zones             | Header       | Medium | Medium | ✅ Done — 16 Sep 2026 | [performance/reivew-items](https://github.com/tommyarmstrong/football-team-organizer/pull/159) |
| 4.3     | Restructure `getPrimaryClub` to avoid chained awaits  | Header       | Medium | Low    | ✅ Done — 16 Sep 2026 | [performance/reivew-items](https://github.com/tommyarmstrong/football-team-organizer/pull/159) |
| **5.1** | Composite RPC or SQL aggregation for stats            | Stats page   | High   | High   | ✅ Done — 16 Sep 2026 | [performance/reivew-items](https://github.com/tommyarmstrong/football-team-organizer/pull/157) |
| 5.2     | Combine or defer team page candidate queries          | Team page    | Medium | Medium | ✅ Done — 16 Sep 2026 | [performance/reivew-items](https://github.com/tommyarmstrong/football-team-organizer/pull/159) |
| 5.3     | Composite person-detail query or RPC                  | People page  | Medium | Medium | ✅ Done — 16 Sep 2026 | [performance/reivew-items](https://github.com/tommyarmstrong/football-team-organizer/pull/159) |
| 5.4     | Composite match-detail query                          | Matches page | Medium | Medium | ✅ Done — 16 Sep 2026 | [performance/reivew-items](https://github.com/tommyarmstrong/football-team-organizer/pull/154) |
| 5.5     | Share dashboard data via single cached fetch          | Dashboard    | Low    | Low    | ✅ Done — 16 Sep 2026 | [performance/reivew-items](https://github.com/tommyarmstrong/football-team-organizer/pull/154) |
| 5.6     | Cross-request caching with `unstable_cache` + tags    | All pages    | Medium | Medium | ✅ Done — 16 Sep 2026 | [performance/reivew-items](https://github.com/tommyarmstrong/football-team-organizer/pull/159) |
| 6.1     | Wrap `createClient()` in `React.cache()`              | DB layer     | Medium | Low    | ✅ Done — 16 Sep 2026 | [performance/reivew-items](https://github.com/tommyarmstrong/football-team-organizer/pull/159) |
...
| 6.3     | Select only needed columns from clubs                 | DB layer     | Low    | Low    | ✅ Done — 16 Sep 2026 | [performance/reivew-items](https://github.com/tommyarmstrong/football-team-organizer/pull/154) |
| **6.4** | Push stats aggregation into Postgres                  | DB layer     | Medium | Medium | ✅ Done — 16 Sep 2026 | [performance/reivew-items](https://github.com/tommyarmstrong/football-team-organizer/pull/157) |
| 6.5     | Combine `getNextFixture` fallback into one query      | DB layer     | Low    | Low    | ✅ Done — 16 Sep 2026 | [performance/reivew-items](https://github.com/tommyarmstrong/football-team-organizer/pull/154) |
...
| 8.1     | Remove `background-attachment: fixed`                 | CSS          | Low    | Low    | ✅ Done — 16 Sep 2026 | [performance/reivew-items](https://github.com/tommyarmstrong/football-team-organizer/pull/154) |
| 8.2     | No action needed for `color-mix()`                    | CSS          | —      | —      | N/A                   |                                                                                                |
| 8.3     | Verify font-display; consider lazy-loading Geist Mono | CSS          | Low    | Low    | ✅ Done — 16 Sep 2026 | [performance/reivew-items](https://github.com/tommyarmstrong/football-team-organizer/pull/154) |

### Notes on completed items

**§3.1** — `fto_access=1` (`HttpOnly; Max-Age=300`) is set by middleware whenever `rpc('has_app_access')` returns `true`. Subsequent requests within the 5-minute window skip the RPC entirely. The cookie is carried through the `/login` → `/dashboard` redirect and is cleared by `signOut()`.

**§2.1** — Middleware now runs a single-row `SELECT colour FROM clubs` (RLS-filtered) whenever the `club_colour_hint` cookie is absent or invalid. The cookie lasts 30 days so this query only fires on the first authenticated request. `ClubColourBinder` is kept as a mid-session fallback updater.

**§1.1** — `GET /auth/session/verify?next=<path>` replaces the client-initiated `POST /auth/session/bootstrap`. After `signInWithPassword` the login form redirects the browser to this route; middleware runs on the same request (§3.1 + §2.1 + §3.2 activate here), and the verify route calls `verifySignedInPersonAccess()` server-side then redirects to `next`. Login now has one client-initiated call followed by one server hop instead of two sequential browser↔server round-trips. The 150 ms colour-resolution timeout is removed from the login critical path entirely.

**§1.2** — `verifySignedInPersonAccess()` now calls `supabase.auth.getSession()` instead of `supabase.auth.getUser()`. Middleware already validated and refreshed the JWT via `getUser()` on the same request, so `getSession()` reads the session from the cookie store without a second network round-trip to the Supabase Auth server. This halves auth network latency for the verify route and any server action that calls `assertPersonMayRemainSignedIn`.
...
**§5.1 + §6.4** — `get_team_stats(p_team_id uuid)` Postgres function (migration `20260916100000_get_team_stats_rpc.sql`) replaces the 5 heavy stats-page functions — `getGoalsByPlayerStats`, `getAssistsByPlayerStats`, `getPlayerOfTheMatchByPlayerStats`, `getMatchesPlayedByPlayerStats`, `getResultsOverTime` — which collectively issued ~11 DB calls. The function uses CTEs (`played_matches`, `goals_by_player`, `assists_by_player`, `potm_by_player`, `matches_played_by_player`, `results_over_time`) to aggregate everything in one Postgres round-trip. Aggregation (GROUP BY, COUNT, ORDER BY) now happens in SQL rather than JavaScript, so only ~5–20 rows are transferred per section instead of hundreds of raw event rows. Per-event competition arrays are preserved inside `jsonb_agg` so the existing client-side competition filter continues to work without changes. The new `getAllTeamStats(teamId)` TypeScript function (wrapped in `React.cache()`) is called once per stats page load; `StatsBody` now makes 2 parallel calls — `getAllTeamStats` and `listCompetitions` — instead of 7. `SECURITY INVOKER` is used so RLS is evaluated for the calling user.

**§1.3** — `findPersonForAuthUserId()` uses the request-scoped RLS client (`createClient()`) instead of the service-role admin client. `people_select` already allows `auth_user_id = auth.uid()`, so the extra admin constructor was unnecessary. `findPersonForVerifiedEmail()` still uses the admin client: that lookup is the unlinked-person email fallback and is not visible under RLS.

**§2.2** — `ClubColourBinder` now uses `useEffect` instead of `useLayoutEffect`. The cookie from §2.1 still applies colour on the first HTML byte; this effect is only the mid-session fallback and no longer blocks paint.

**§3.3** — The proxy matcher now also skips `/api/*`, `/auth/callback`, and `/auth/confirm`. Those routes exchange their own session. `/auth/session/verify`, `/auth/invite`, and `/auth/forgot-password` still run middleware so post-login cookies and logged-in redirects keep working.

**§5.4** — `getMatchDetail(id)` loads the match with nested `goals`, `cards`, `match_players`, and `match_periods` in one PostgREST select. The match page then only fans out the team roster (needed for add-player UI) and the postcard payload. That cuts the detail waterfall from 2 + 6 queries to 2 + 2.

**§5.5** — `getDashboardData(teamId)` (wrapped in `React.cache()`) `Promise.all`s next fixture, last result, form, competitions, and leaderboards. Each dashboard Suspense section still renders on its own; they share one in-flight fetch instead of overlapping match/goal queries.

**§6.3** — `listVisibleClubs` and `get_viewer_context()` (migration `20260916200000_viewer_context_club_summary.sql`) select `id, name, colour, icon_url` only. Club view/edit pages load the full row with `getClub(id)` (already tagged-cache).

**§6.5** — `getNextFixture()` issues one `status = scheduled` query ordered by date and picks the first row on/after today, otherwise the earliest scheduled row. The previous upcoming-then-fallback waterfall is gone.

**§8.1** — The pitch-pattern background is painted on `body::before` (`position: fixed`) instead of `background-attachment: fixed` on `body`, so scrolling no longer forces a full-background repaint on mobile.

**§8.3** — Outfit and Barlow Condensed set `display: "swap"` explicitly. Geist Mono is no longer loaded globally; `font-mono` uses the system stacked fallback for the two small monospace UI spots.
