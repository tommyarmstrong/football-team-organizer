-- England demo seed: FA club + England Men (1965/66, 1995/96, 2025/26) + England Women 2021/22.
--
-- Can be applied alone, or after supabase/seed.sql. Uses the same fixed club and
-- John Hall manager ids as seed.sql (upserts), so stacking is safe and idempotent.
-- If both are loaded, run seed.sql first, then this file.
--
-- Seeded domain data:
--   - 1 club (The Football Association) with colour, crest URL, established year, about
...
--   - Full match-day squads on every Euro 96 (22) and World Cup 2026 (26) match
...
with upserted_club as (
  ...
  values (
    ...
    'https://www.thefa.com',
    '#CF081F',
    'https://upload.wikimedia.org/wikipedia/en/thumb/4/47/England_national_football_team_crest.svg/240px-England_national_football_team_crest.svg.png',
    1863,
    'Governing body of football in England. Demo club for local and staging environments.'
  )
...
-- Match-day squads: full Euro 96 squad (22) on every match.
delete from public.match_players
where match_id in (
  'e0000003-0000-4000-8000-000000000001',
  'e0000003-0000-4000-8000-000000000002',
...
select m.id, tp.player_id
from (
  values
    ('e0000003-0000-4000-8000-000000000001'::uuid),
    ('e0000003-0000-4000-8000-000000000002'::uuid),
    ('e0000003-0000-4000-8000-000000000003'::uuid),
    ('e0000003-0000-4000-8000-000000000004'::uuid),
    ('e0000003-0000-4000-8000-000000000005'::uuid)
) as m(id)
cross join public.team_players tp
where tp.team_id = 'bbbbbbbb-bbbb-cccc-dddd-000000000096'
...
-- Match-day squads: full World Cup 2026 squad (26) on every match.
delete from public.match_players
where match_id in (
  'e0000004-0000-4000-8000-000000000001',
  'e0000004-0000-4000-8000-000000000002',
...
from (
  values
    ('e0000004-0000-4000-8000-000000000001'::uuid),
    ('e0000004-0000-4000-8000-000000000002'::uuid),
    ('e0000004-0000-4000-8000-000000000003'::uuid),
    ('e0000004-0000-4000-8000-000000000004'::uuid),
    ('e0000004-0000-4000-8000-000000000005'::uuid),
    ('e0000004-0000-4000-8000-000000000006'::uuid),
    ('e0000004-0000-4000-8000-000000000007'::uuid),
    ('e0000004-0000-4000-8000-000000000008'::uuid)
) as m(id)
cross join public.team_players tp
where tp.team_id = 'bbbbbbbb-bbbb-cccc-dddd-000000002026'
on conflict (match_id, player_id) do nothing;
