-- Production still had matches / match_players / periods / cards write
-- policies on can_edit_team (coach + club management only). Guardian
-- assistants could add goals (can_edit_match_goals) but squad deletes were
-- silently blocked and match status updates returned PGRST116 from
-- .update().select().single() when RLS hid the row.
--
-- Align write policies with can_edit_match_day (already defined) and protect
-- player-of-the-match columns for coach/management only.
...
-- Matches: create/update/delete fixtures (POTM columns protected below).
